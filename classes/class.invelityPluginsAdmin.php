...
class InvelityPluginsAdmin
{
    ...
    private function getRemotePluginsData()
    {
        ...
        if (!$invelityPluginsDescription) {
            $response = '';
            try {
                $query = esc_url_raw(add_query_arg([], 'https://licenses.invelity.com/plugins/invelitypluginsdata.json'));
                $response = wp_remote_get($query, ['timeout' => 2, 'sslverify' => false]);
                $response = wp_remote_retrieve_body($response);
                if (!$response && file_exists(plugin_dir_path(__FILE__) . '../json/invelitypluginsdata.json')) {
                    $response = file_get_contents(plugin_dir_path(__FILE__) . '../json/invelitypluginsdata.json');
                }
            } catch (Exception $e) {
    ...
    private function getRemoteAd()
    {
        ...
        if (!$invelityPluginsad) {
            $response = '';
            try {
                $query = esc_url_raw(add_query_arg([], 'https://licenses.invelity.com/plugins/invelityad.json'));
                $response = wp_remote_get($query, ['timeout' => 2, 'sslverify' => false]);
                $response = wp_remote_retrieve_body($response);
                if (!$response && file_exists(plugin_dir_path(__FILE__) . '../json/invelityad-global.json')) {
                    $response = file_get_contents(plugin_dir_path(__FILE__) . '../json/invelityad-global.json');
                }
            } catch (Exception $e) {
...
//            set_transient('invelity-plugins-ad', $response, 300);/*5 min*/
    ...
    {
        wp_register_style('invelity-plugins-main-admin-css', $this->launcher->getPluginUrl() . 'assets/css/invelity-plugins-main-admin.css', [], '1.0.0');
    ...
    {
        $path = ABSPATH . 'wp-content/plugins/' . $pluginRemoteData->wp_url;
